  ...
  if (state.questions.length === 0) {
    el.innerHTML = `
      <div style="padding:3rem;text-align:center;color:var(--text-3)">
        ...
        <div style="font-size:0.875rem;margin-top:0.25rem">Klik <strong>+ Tambah Pertanyaan</strong> untuk mulai membuat form feedback.</div>
      </div>`;
    return;
  }
      ...
      <div class="question-card">
        <div style="display:flex;align-items:flex-start;gap:1rem">
          <div style="display:flex;flex-direction:column;gap:0.25rem;padding-top:0.1rem">
            <button class="icon-btn" onclick="moveQuestion(${q.id},-1)" title="Naik"  ${i === 0 ? 'disabled' : ''}>▲</button>
            <button class="icon-btn" onclick="moveQuestion(${q.id},1)"  title="Turun" ${i === state.questions.length - 1 ? 'disabled' : ''}>▼</button>
          </div>
          ...
          <div style="display:flex;gap:0.4rem;flex-shrink:0">
            ...
            <button class="btn-delete" onclick="deleteQuestion(${q.id})" title="Hapus">🗑️</button>
          </div>
        </div>
      </div>`;
  }).join('');
...
async function moveQuestion(id, dir) {
  ...
  const b = state.questions[newIdx];
  try {
...
async function deleteQuestion(id) {
  ...
  try {
    await adminDB.deleteQuestion(id);
    await loadQuestions();
  ...
  if (id !== null) {
    ...
    setTimeout(() => {
      if (q.type === 'scale') {
        ...
        if (container) {
          container.innerHTML = '';
          (Array.isArray(q.options) ? q.options : []).forEach(opt => addOptionField(opt));
        }
      }
    }, 30);
  } else {
...
function addOptionField(value = '') {
  ...
  div.innerHTML = `
    ...
    <button type="button" onclick="this.parentElement.remove()"
      style="background:none;border:none;cursor:pointer;color:var(--danger);font-size:1.1rem;padding:0 0.25rem;line-height:1">✕</button>`;
...
async function saveQuestion() {
  ...
  try {
    if (state.editingId !== null) {
      await adminDB.updateQuestion(state.editingId, data);
    } else {
      data.sort_order = state.questions.length;
      await adminDB.insertQuestion(data);
    }
    closeModal();
    await loadQuestions();
  } catch (err) {
    ...
    if (btn) { btn.disabled = false; btn.textContent = 'Simpan Pertanyaan'; }
  }
}
...
async function loadResponses() {
  ...
  try {
    state.responses = await adminDB.getResponses();
    renderResponses();
  } catch (err) {
    if (el) el.innerHTML = `<div style="padding:1rem;color:var(--danger)">⚠️ ${err.message}</div>`;
  }
  ...
  if (state.responses.length === 0) {
    el.innerHTML = `
      <div style="padding:3.5rem;text-align:center;color:var(--text-3)">
        ...
        <div style="font-size:0.875rem;margin-top:0.25rem">Respons akan muncul setelah peserta mengisi form feedback.</div>
      </div>`;
    return;
  }
...
function renderSummary() {
  ...
  const cards = state.questions.map(q => {
    const answers = state.responses
      .flatMap(r => (r.feedback_answers || []).filter(a => a.question_id === q.id).map(a => a.answer_value))
      .filter(Boolean);
    const meta = TYPE_META[q.type] || { label: q.type, icon: '❓' };

    if (!answers.length) return `
      <div class="summary-card">
        <div class="summary-q">${q.question_text}
          <span class="type-badge" style="margin-left:0.4rem">${meta.icon} ${meta.label}</span>
        </div>
        <div style="color:var(--text-3);font-size:0.85rem;padding-top:0.25rem">Belum ada jawaban.</div>
      </div>`;
    ...
    if (q.type === 'scale') {
      ...
      const counts = {};
      for (let n = q.scale_min; n <= q.scale_max; n++) counts[n] = 0;
      ...
      return `
        <div class="summary-card">
          <div class="summary-q">${q.question_text}
            <span class="type-badge" style="margin-left:0.4rem">⭐ Skala</span>
          </div>
          <div style="font-size:1.75rem;font-weight:800;color:var(--primary);margin-bottom:0.75rem">
            ${avg} <span style="font-size:0.9rem;font-weight:400;color:var(--text-3)">/ ${q.scale_max}</span>
          </div>
          ${bars}
          <div style="font-size:0.78rem;color:var(--text-4);margin-top:0.5rem">${nums.length} jawaban</div>
        </div>`;
    }
    ...
    if (['multiple_choice', 'checkbox', 'dropdown'].includes(q.type)) {
      ...
      const bars   = Object.entries(counts).map(([opt, cnt]) => `
        <div style="margin-bottom:0.6rem">
          <div style="display:flex;justify-content:space-between;font-size:0.83rem;margin-bottom:0.2rem">
            <span>${opt}</span>
            <span style="color:var(--text-3)">${cnt} (${total ? Math.round(cnt/total*100) : 0}%)</span>
          </div>
      ...
      return `
        <div class="summary-card">
          ...
          ${bars}
          <div style="font-size:0.78rem;color:var(--text-4);margin-top:0.25rem">${total} jawaban</div>
        </div>`;
    }
    ...
    return `
      <div class="summary-card">
        ...
        <div style="max-height:180px;overflow-y:auto;display:flex;flex-direction:column;gap:0.4rem">
          ${answers.map(a => `
            <div style="padding:0.4rem 0.65rem;background:var(--bg-2,#f8fafc);border-radius:6px;
                        font-size:0.85rem;border:1px solid var(--border)">${a}</div>`).join('')}
        </div>
        <div style="font-size:0.78rem;color:var(--text-4);margin-top:0.5rem">${answers.length} jawaban</div>
      </div>`;
  }).join('');

  return `
    <h4 style="margin:0 0 1rem;font-size:1rem">📊 Ringkasan Jawaban</h4>
          ...
          ${state.responses.map((r, i) => `
            <tr>
              ...
              <td style="font-size:0.8rem;color:var(--text-3)">
                ${new Date(r.created_at).toLocaleString('id-ID', { day:'numeric', month:'short', year:'numeric', hour:'2-digit', minute:'2-digit' })}
              </td>
              <td>
                <button class="btn-edit" onclick="toggleDetail(${r.id}, this)">👁 Detail</button>
              </td>
            </tr>
            <tr id="detail_${r.id}" style="display:none;background:var(--bg-2,#f8fafc)">
              <td colspan="5" style="padding:0.75rem 1.25rem">
                ${renderAnswerDetail(r)}
              </td>
            </tr>`).join('')}
        </tbody>
      </table>
...
function renderAnswerDetail(r) {
  ...
  return `<div style="display:grid;gap:0.6rem">` + answers.map(a => {
    ...
    return `
      <div>
        <div style="font-size:0.78rem;color:var(--text-3);margin-bottom:0.15rem">
          ${q ? q.question_text : 'Pertanyaan #' + a.question_id}
        </div>
        <div style="font-weight:500;font-size:0.9rem">${a.answer_value || '—'}</div>
      </div>`;
  }).join('') + '</div>';
}
